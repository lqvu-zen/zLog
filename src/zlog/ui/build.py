...
def build_widgets(win) -> None:
    ...
    win.package_box.lineEdit().setPlaceholderText("Package, e.g. com.example.app")
    win.load_pkgs_btn = QPushButton("Load")
    win.apply_pkg_btn = QPushButton("Apply")
    win.clear_pkg_btn = QPushButton("Clear pkg")
    win.focus_app_btn = QPushButton("Focus App…")
    win.focus_app_btn.setToolTip("Pick a running Windows app to focus the view on")
    ...
    win.clear_filters_btn.setToolTip("Reset all filters (level, search, tag, package, time…)")
