...
def _message_fraction(usable, cw, show):
    fixed = _fixed_px(cw)
    tag_w, proc_w = plan_tag_proc_widths(usable, cw, True, show, fixed)
...
def test_flexible_columns_use_natural_width_when_there_is_room():
    cw = 9
    tag_w, proc_w = plan_tag_proc_widths(
        4000, cw, show_tag=True, show_proc=True, fixed_px=_fixed_px(cw)
    )
...
def test_flexible_columns_shrink_when_narrow():
    cw = 9
    tag_w, proc_w = plan_tag_proc_widths(
        1100, cw, show_tag=True, show_proc=True, fixed_px=_fixed_px(cw)
    )
...
def test_tag_hidden_collapses_to_zero_width_and_no_gap():
    cw = 9
    tag_w, proc_w = plan_tag_proc_widths(
        2000, cw, show_tag=False, show_proc=True, fixed_px=_fixed_px(cw)
    )
    assert tag_w == 0
    assert proc_w == _PROC_W * cw  # proc is unaffected by tag being off


def test_both_hidden_is_zero():
    cw = 9
    tag_w, proc_w = plan_tag_proc_widths(
        2000, cw, show_tag=False, show_proc=False, fixed_px=_fixed_px(cw)
    )
    assert (tag_w, proc_w) == (0, 0)
...
def test_col_widths_collapse_when_never_populated(qapp):
    ...
    model = LogTableModel()
    model.append_entries([LogEntry("t", "", "", "I", "", "debug-output style line")])

    d = LogItemDelegate()
    fm = QFontMetrics(QFont())
    cw = fm.horizontalAdvance("M") or 8
    time_w, pid_w, tag_w, proc_w = d._col_widths(0, 1600, cw, model, fm)
    assert pid_w == 0
    assert tag_w == 0
    assert time_w > 0  # Time is never auto-hidden


def test_col_widths_show_once_a_row_has_them(qapp):
    from PySide6.QtGui import QFont, QFontMetrics
    ...
    model = LogTableModel()
    model.append_entries([LogEntry("t", "", "", "I", "", "no pid/tid/tag yet")])
    d = LogItemDelegate()
    fm = QFontMetrics(QFont())
    cw = fm.horizontalAdvance("M") or 8
    assert d._col_widths(0, 1600, cw, model, fm)[1] == 0  # pid_w still 0

    model.append_entries([LogEntry("t", "123", "", "I", "Tag", "now it has pid + tag")])
    _time_w, pid_w, tag_w, _proc_w = d._col_widths(0, 1600, cw, model, fm)
    assert pid_w > 0
    assert tag_w > 0


def test_paint_with_no_pid_tid_tag_does_not_crash(qapp):
    """Smoke-test the auto-hidden (0-width) segment path through paint()."""
    from PySide6.QtCore import QRect
    from PySide6.QtGui import QFont, QImage, QPainter
    from PySide6.QtWidgets import QStyleOptionViewItem
