...
- [ ] `uv run pytest` (print-HTML header/colors/escaping/empty)
...
- **Row cap:** 50k, or paginate into multiple files? Leaning a cap + warning.
- **Orientation/page size:** default A4 portrait, or landscape (log lines are
  wide)? Leaning landscape by default, since wrapping long messages hurts
  readability more than the unusual orientation does.
- Add "Print…" alongside, given it's nearly free? Leaning yes if the plumbing
  turns out identical.
