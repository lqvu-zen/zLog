...
| Plan | Status | Summary |
|---|---|---|
| [network-log-source.md](network-log-source.md) | Done | TCP listener source — any socket-speaking process becomes a log source, no adb/file needed |
| [json-field-filter.md](json-field-filter.md) | Done (scope cut — see plan) | JSON auto-fields + a `field:` query token, closing the filter gap `regex-extract-columns.md` left open |
...
| [crash-symbolication.md](crash-symbolication.md) | Done | Deobfuscate Java/Kotlin (ProGuard/R8 mapping.txt) and symbolicate native (NDK addr2line) crash traces; new Symbol bar under the device bar |
| [release-1.4.0.md](release-1.4.0.md) | Done | Cut the 1.4.0 release — crash symbolication |
...
| [release-1.1.1.md](release-1.1.1.md) | Done | Cut 1.1.1 — ships the fix for a shipped bug that makes the Windows flow unreachable |
| [release-1.2.0.md](release-1.2.0.md) | Done | Cut the 1.2.0 release — user-defined log formats + export/import |
...
| [ci-shuffled-order.md](ci-shuffled-order.md) | Done | One CI job in shuffled order, to surface order-dependent tests deliberately |
...
| [filter-chips.md](filter-chips.md) | Done | Active query tokens as removable chips under the query bar |
| [bookmark-labels.md](bookmark-labels.md) | Done | Name bookmarks + a Bookmarks dock to jump between them |
| [large-file-progress.md](large-file-progress.md) | Done | Stream big `.log` opens in the background with a cancelable progress dialog |
...
| [query-autocomplete.md](query-autocomplete.md) | Done | Context-aware query-bar completion popup: keys + level names + live tag/pid/proc values |
...
| [column-header-labels.md](column-header-labels.md) | Done | Thin label strip above the log showing each segment's name (Time/PID·TID/Tag/Lvl/Message) |
...
| [usable-without-adb.md](usable-without-adb.md) | Done | Bug: with no adb installed, refresh_devices returns early so "This PC" never lists and Start is dead |
| [bundle-adb.md](bundle-adb.md) | Done | Offer to fetch adb when missing (licensing rules out bundling); README covers manual install |
